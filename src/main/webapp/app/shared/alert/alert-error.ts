...
export class AlertError implements OnDestroy {
  ...
  constructor() {
    this.errorListener = this.eventManager.subscribe('sistemKepegawaianApp.error', (response: EventWithContent<unknown> | string) => {
    ...
    this.httpErrorListener = this.eventManager.subscribe(
      'sistemKepegawaianApp.httpError',
  ...
  private handleFieldsError(httpErrorResponse: HttpErrorResponse): void {
    ...
    for (const fieldError of fieldErrors) {
      ...
      const fieldName: string = this.translateService.instant(`sistemKepegawaianApp.${fieldError.objectName as string}.${convertedField}`);
