...
import { sampleWithRequiredData } from '../penggajian.test-samples';
...
describe('Penggajian Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: Penggajian;
  let fixture: ComponentFixture<Penggajian>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Penggajian);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 20594 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.penggajians()[0]).toEqual(expect.objectContaining({ id: 20594 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 20594 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.penggajians()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 20594 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to penggajianService', () => {
      const entity = { id: 20594 };
      vitest.spyOn(service, 'getPenggajianIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPenggajianIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.penggajiansParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.penggajiansParams()).toMatchObject(expect.objectContaining({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] }));
  });
