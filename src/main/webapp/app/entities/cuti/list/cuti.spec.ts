...
import { sampleWithRequiredData } from '../cuti.test-samples';
...
describe('Cuti Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: Cuti;
  let fixture: ComponentFixture<Cuti>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Cuti);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 2717 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.cutis()[0]).toEqual(expect.objectContaining({ id: 2717 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 2717 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.cutis()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 2717 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to cutiService', () => {
      const entity = { id: 2717 };
      vitest.spyOn(service, 'getCutiIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCutiIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.cutisParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.cutisParams()).toMatchObject(expect.objectContaining({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] }));
  });
