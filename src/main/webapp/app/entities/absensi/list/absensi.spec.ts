...
import { sampleWithRequiredData } from '../absensi.test-samples';
...
describe('Absensi Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: Absensi;
  let fixture: ComponentFixture<Absensi>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Absensi);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 18626 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.absensis()[0]).toEqual(expect.objectContaining({ id: 18626 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 18626 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.absensis()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 18626 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to absensiService', () => {
      const entity = { id: 18626 };
      vitest.spyOn(service, 'getAbsensiIdentifier');
      const id = comp.trackId(entity);
      expect(service.getAbsensiIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.absensisParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.absensisParams()).toMatchObject(expect.objectContaining({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] }));
  });
